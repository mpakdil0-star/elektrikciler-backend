...
export const jobService = {
  async createJob(data: CreateJobData) {
    ...
    const jobPost = await prisma.jobPost.create({
      data: {
        ...
        category,
        subcategory,
        // serviceCategory is NOT a JobPost DB field - used only for notification routing
    ...
    // Notify nearby electricians (pass serviceCategory for routing)
    try {
      this.notifyNearbyElectricians({ ...jobPost, serviceCategory });
    } catch (error) {
  ...
  async notifyNearbyElectricians(job: any) {
    ...
    try {
      ...
      if (isDatabaseAvailable) {
        // Query database for matching electricians
        ...
        for (const elec of electricians) {
          ...
          // Kaydet (Service hem DB hem Mock kontrolü yapar ama burada DB hedefliyoruz)
          saveNotification({
            userId: elec.id,
            type: 'new_job_available',
            title: 'Yeni İş İlanı!',
      ...
      // 3. MOCK PERSISTENCE (If not handled by DB loop or if there are mock users)
      const { getAllMockUsers } = require('../utils/mockStorage');
      const allMockUsers = getAllMockUsers();

      Object.entries(allMockUsers).forEach(([userId, userData]: [string, any]) => {
        if (userId === job.citizenId) return;
        if (userData.userType !== 'ELECTRICIAN') return;

        // Category match
        if (userData.serviceCategory !== serviceCategory) return;

        // Location match
        const hasLocationMatch = userData.locations?.some((loc: any) =>
          loc.city?.toLowerCase() === city?.toLowerCase() &&
          (!district || !loc.district || loc.district === 'Tüm Şehir' || loc.district === 'Merkez' || loc.district?.toLowerCase() === district.toLowerCase())
        );
        ...
        if (hasLocationMatch) {
          saveNotification({
            ...
            type: 'new_job_available',
            title: 'Yeni İş İlanı! ⚡',
            message: `Bölgenizde yeni ilan verildi: ${job.title}`,
            relatedType: 'JOB',
            relatedId: job.id,
          });
        }
  ...
  async getJobs(filters: {
    ...
    try {
      ...
      if (city && districts && districts.length > 0) {
        // City + multiple districts: use OR for each district (JSON fields don't support 'in')
        where.OR = districts.map((d: string) => ({
          AND: [
            { location: { path: ['city'], equals: city } },
            { location: { path: ['district'], equals: d } },
          ],
        }));
      } else if (city && district) {
        // City + single district
        where.AND = [
          { location: { path: ['city'], equals: city } },
          { location: { path: ['district'], equals: district } },
        ];
      } else if (city) {
        // City only
        where.location = {
